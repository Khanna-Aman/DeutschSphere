...
DeutschSphere is designed to maximize vocabulary acquisition through distraction-free, clinical spaced repetition. The application focuses on high-signal cognitive retention of **3,054 validated German vocabulary words** via mathematical memory modeling (an FSRS-inspired scheduler based on the FSRS-5 model — see `js/fsrs.js` for its documented simplifications).
...
| **Visual Assets** | **Active** | High-fidelity WebP visual aids (<10KB) completed for A1 (637/684, 93%) and A2 (580/582, ~100%). B1 in progress (371/1,788, 21%). |
...
* **B1 Vocabulary Backfill**: B1 is **under active development** — currently ~85% of the official (cumulative) Goethe B1 *Wortliste* (~87% of B1 nouns). **Phase 1 shipped 2026-07-03**: 61 everyday entries added (52 nouns incl. *Gefühl, Gefahr, Boot, Bär, Demokratie* + 9 verbs incl. *aufgeben, abnehmen, atmen*), each source-grounded (gender/plural from the official list, null where unattested) with an original LanguageTool-gated example. **Next phases**: remaining ~90 alphabetical nouns, thematic groups (days, months, seasons), feminine `-in` doublets + regional-variant policy.
* **B1 Asset Rollout**: Complete the final integration of audited 3D glassmorphic WebP assets for the remaining B1 entries (371/1,788 done, 1,417 outstanding) following strict anti-bleeding checks.
...
* **Active Vocabulary Capacity**: 3,054 entries across CEFR A1 (684), A2 (582), and B1 (1,788). B1 is **under active development** — currently ~90% of the official (cumulative) Goethe B1 *Wortliste* (~89% of B1 nouns; everyday verb/adjective gaps closed in Phase 2); remaining thematic groups, `-in` doublets and regional variants are being backfilled in post-launch releases.
