...
1. **Zero Runtime Dependencies**: No runtime framework, bundler, or npm package ships to the client — app code stays vanilla ES6 modules. The build-time tools (Tailwind CLI, esbuild for `index.css`/`index.src.css`, `fonttools` for icon subsetting) run on demand to regenerate committed, pre-generated output files (see [DEVELOPMENT.md → Styling](DEVELOPMENT.md#styling-precompiled-tailwind--minified-hand-authored-css)). Do not add a runtime build step or commit `node_modules`.
